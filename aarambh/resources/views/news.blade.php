...
<div class="row">
    ...
    <div class="col-6 text-end px-0 px-lg-3">
        <a href="{{route('addnews')}}" class="btn btn-primary btn-sm px-3"><i class='bx bx-plus'></i>Add</a>
        {{-- <button type="button" onclick="ajaxModal('{{ url('ajaxModal/modal_files.add_news') }}', sendObj({'_token' : '{{ csrf_token() }}'}))" class="btn btn-primary btn-sm px-3"><i class='bx bx-plus'></i>Add</button> --}}
    </div>
</div>
<hr />
                ...
                <tbody>
                    <tr dir-paginate="item in users | filter: q | itemsPerPage: usersPerPage" total-items="totalUsers" current-page="pagination.current">
                        ...
                        <td>
                            ...
                            <img style="width: 2rem" class="rounded" src="{{asset('')}}/@{{ item.image }}" alt="@{{ item.title}}">
                        </td>
                        <td>@{{ item.title }}</td>
                        ...
                        <td>
                            <div class="dropdown dropstart d-flex order-actions">
                                ...
                                <ul class="dropdown-menu">
                                    <li class="py-1">
                                        <a class="dropdown-item px-3" style="all: unset; cursor: pointer;" href="{{ route('addnews') }}/change_status/@{{ item.id }}">@{{ item.status == 1 ? "Mark Inactive" : "Mark Active" }}</a>
                                    </li>
                                    <li class="py-1">
                                        <a href="{{route('updatenews')}}/@{{ item.id }}" class="dropdown-item px-3" style="all: unset; cursor: pointer;">Edit</a>
                                    </li>
                                    <li class="py-1">
                                        <a class="dropdown-item px-3" style="all: unset; cursor: pointer;" href="{{ route('addnews') }}/delete/@{{ item.id }}">Delete</a>
                                    </li>
