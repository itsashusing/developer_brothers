...
<div class="row">
    ...
    <div class="col-6 text-end px-0 px-lg-3">
        <button type="button" onclick="ajaxModal('{{ url('ajaxModal/modal_files.add_role_users') }}', sendObj({'_token' : '{{ csrf_token() }}'}))" class="btn btn-primary btn-sm px-3"><i class='bx bx-plus'></i>Add</button>
    </div>
</div>
<hr />
                ...
                <tbody>
                    <tr dir-paginate="item in users | filter: q | itemsPerPage: usersPerPage" total-items="totalUsers" current-page="pagination.current">
                        ...
                        <td>
                            ...
                            <span ng-if="item.permission.length <= 0" class="badge rounded-pill bg-danger">Permission
                                Denied</span>
                            <span ng-if="item.permission.length > 0" class="badge rounded-pill bg-success">Permission
                                Granted</span>
                        </td>
                        <td>
                            <div class="dropdown dropstart d-flex order-actions">
                                ...
                                <ul class="dropdown-menu">
                                    <li class="py-1">
                                        <a class="dropdown-item px-3" style="all: unset; cursor: pointer;" href="{{ route('role_users') }}/change_status/@{{ item.id }}">@{{ item.status == 1 ? "Mark Inactive" : "Mark Active" }}</a>
                                    </li>
                                    <li class="py-1">
                                        <a href="javascript:;" class="dropdown-item px-3" style="all: unset; cursor: pointer;" ng-click="ajaxModal('{{ url('ajaxModal/modal_files.update_role_user') }}', sendObj({'_token' : '{{ csrf_token() }}', 'param1' : item.id}))">Edit</a>
                                    </li>
                                </ul>
                            </div>
                        </td>
