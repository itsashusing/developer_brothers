...
<div class="row">
    ...
    <div class="col-6 text-end px-0 px-lg-3" id="permission_country_create">
        <button type="button" onclick="ajaxModal('{{ url('ajaxModal/modal_files.add_subject') }}', sendObj({'_token' : '{{ csrf_token() }}'}))" class="btn btn-primary btn-sm px-3"><i class='bx bx-plus'></i>Add</button>
    </div>
</div>
<hr />
            ...
            <table class="table dataTable no-footer" style="width:100%; border: 1px solid #e9ecef">
                ...
                <tbody>
                    <tr dir-paginate="item in users | filter: q | itemsPerPage: usersPerPage" total-items="totalUsers" current-page="pagination.current">
                        ...
                        <td><img style="width: 2rem" class="rounded" src="{{asset('')}}/@{{ item.image }}" alt="" srcset=""> </td>
                        <td>@{{ item.subject_name }}</td>
                        <td>@{{ item.sub_category.category.category_name}}</td>
                        ...
                        <td>
                            <div class="dropdown dropstart d-flex order-actions">
                                ...
                                <ul class="dropdown-menu">
                                    <li class="py-1">
                                        <a class="dropdown-item px-3" style="all: unset; cursor: pointer;" href="{{ route('subject') }}/change_status/@{{ item.id }}">@{{ item.status == 1 ? "Mark Inactive" : "Mark Active" }}</a>
                                    </li>
                                    <li class="py-1">
                                        <a href="javascript:;" class="dropdown-item px-3" style="all: unset; cursor: pointer;" ng-click="ajaxModal('{{ url('ajaxModal/modal_files.edit_subject') }}', sendObj({'_token' : '{{ csrf_token() }}', 'id' : item.id}))">Edit</a>
                                    </li>
                                    <li class="py-1">
                                        <a class="dropdown-item px-3" style="all: unset; cursor: pointer;" href="{{ route('subject') }}/delete/@{{ item.id }}">Delete</a>
                                    </li>
                                </ul>
                            </div>
                        </td>
